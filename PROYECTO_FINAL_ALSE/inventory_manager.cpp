...
void inventory_manager::on_btn_agregar_clicked()
{
    ...
    try {
        ...
        if(Cpnt->procesarCompra(name, quantity)) { // Si no se puede ejecutar procesarCompra, esta retorna false y manda un mensaje de error
            ...
            if(!dbManager->addRecord(name, type, quantity, location, purchase_date)) { // Si ocurre también un error a la hora de registrar una compra, addRecord retorna false y se arroja un mensaje de error
                ui->statusLabel->setText("Error al añadir registro de compra");
        ...
        else { // Si no se puede ejecutar procesarCompra, esta retorna false y manda un mensaje de error
            ui->statusLabel->setText("Error en transacción, Stock insuficiente");
            qDebug() << "Error en transacción";
        }

        db.commit(); // Commit verifica ambas transacciones (procesarCompra y addRecord)
